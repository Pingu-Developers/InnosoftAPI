...
describe("_________________Innosoft Integration Tests_________________", function() {

    /* EXECUTED BEFORE ALL TESTS */
    before((done) => {
        console.log('---------- Start Server Infrastructure ----------');
        try {
            execSync("docker-compose -f tests/docker-compose-test.yaml pull db innoApi innoChatDb");
            execSync("docker-compose -f tests/docker-compose-test.yaml up -d db innoApi innoChatDb");
    ...
    after((done) => {
        console.log('---------- Stop Server Infrastructure ----------');
        try {
            socket.disconnect();
            execSync("docker-compose -f tests/docker-compose-test.yaml down");
            done();
        } catch (err) {
            console.log(err);
            done(err);
